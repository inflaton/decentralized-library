...
export default {
  ...
  data() {
    return {
      ...
      email: "", // variable binded with the input field: status
  ...
  computed: {
    /**
     * It disables the submit button when the the name or email are not filled
      ...
      window.bc.contract().registerUser(
        ...
        (err, txHash) => {
          ...
          if (err) {
            ...
            Event.$emit("user_registered", txHash);
